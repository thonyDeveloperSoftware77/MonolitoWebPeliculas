...
export default function Favorite({ user, uid, setUpdate}) {
    ...
    const buscarPelicula = async (id) => {
        ...
        await user._favoritos.map((movie) => {
        ...
        <main >
            {peliculas.length > 0 ?
                <div>
                    ...
                    <ModalMovie setUpdate={setUpdate} isOpen={isOpen} uid={uid} onClose={onClose} pelicula={movieSelected} generos={generos} />

                </div>

                : <div>

                    <h1 className="text-center">No tienes peliculas favoritas</h1>
                </div>}
